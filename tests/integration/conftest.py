...
@pytest.fixture(scope="session")
def ipsec_server() -> str:
    return _require_env("IPSEC_SERVER")


@pytest.fixture(scope="session")
def ipsec_psk() -> str:
    return _require_env("IPSEC_PSK")
...
@pytest.fixture
def ipsec_client_config(ipsec_server: str) -> Path:
    """IPsec client config generated by the strongSwan server at startup.

    The strongswan-server container writes client swanctl.conf to /shared/ipsec-client.conf
    which is mounted at /shared/ipsec/ipsec-client.conf in the test-runner.
    """
    shared_config = Path("/shared/ipsec/ipsec-client.conf")
    if shared_config.exists():
        return shared_config
    pytest.skip("IPsec client config not found at /shared/ipsec/ipsec-client.conf")
